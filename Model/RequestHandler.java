...
public class RequestHandler implements Handler {
    ...
    private ByteBuffer requestByteBuffer = null;//存放HTTP请求的缓存区
    private boolean requestReceived = false;  //表示是否已经收到HTTP请求的所有数据
    ...
    private boolean parse() throws IOException {
        try {
            ...
            System.out.println(request.action()+ "  " + request.version() + "   " +     request.uri());
            return true;
        }  catch (Exception x) {
            response = new Response (Response.Code.BAD_REQUEST,new StringContent(x.toString()));
        }
        return false;
    }
        ...
        try {
            if (request == null) { //如果还没有接收到HTTP请求的所有数据
                ...
                try {
                    ...
                    response.release();
                    response = new Response(Response.Code.NOT_FOUND, new StringContent(x.toString()));
                    response.prepare();
                }
                if (send()) {
                    //如果HTpp响应没有发送完毕，租需要注册写就绪事件
                    //以便在写就绪事件发生时继续发送数据
